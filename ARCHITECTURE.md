...
| `@sparkle/llm-client`        | 单次 LLM chat 执行 + provider + embedding client 运行时；不持有 usage 策略，只发 observation，落库 / 缓存归 LLM 网关装配层，对 persistence 零依赖                                                                                                                                                                                                                              |
...
| `@sparkle/llm-api`           | sparkle-llm 进程契约包（per-producer `xxx-api`，#230/#279）：内部 RPC providers/chat-direct/embed（后两条信封级）+ `/auth/*` 六条 OAuth 管理路由（web 消费）+ console-facing `GET /llm/providers`（provider 列举，web 直连、取代旧 agent 中转）+ LLM 负载核心 schema（`llm-chat`）与 auth 系 schema                                                                            |
...
`agent/runtime/llm-client.ts` 持有 `server.agent.usages`，统一解析 provider/model/thinking、按顺序重试，并保持主 Agent 与镜像摘要共用 `usage=agent`。外层主循环的失败退避仍属于 agent 运行节奏。

`acl/HttpLlmClient` 只负责 HTTP 传输，调用网关的 `chatDirect`。`sparkle-llm` 持有 provider、凭据、单次执行与观测落库；不根据 usage 选模型。每次尝试携带调用方生成的 `trace`（requestId/seq/usage/scene），这些字段只用于日志和 metric，不进入模型请求前缀。网关列举 provider 时不带 usage，agent 自己按偏好排序；管理台直接显示网关的通用列表。
