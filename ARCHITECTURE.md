...
> `@sparkle/image` 是只依赖 sharp 的叶子包（图片归一化：进 LLM 上下文 / vision 前的缩放与长图切片，以及 wire 层 7900px 保险丝，#556），被 agent / napcat / llm-client 消费。
...
| `@sparkle/spire-service`     | 杀塔式卡牌游戏引擎独立进程（`apps/spire`，Fastify，仅 localhost）：纯游戏引擎 + JSON 存档 + 自动对战模拟器，不碰共享 SQLite；agent 经 `HttpSpireClient` 驱动，拆进程让 agent 重启不打断对局（#234）                                                                                                                                                                                                                                       |
| `@sparkle/pixel-service`     | 像素画渲染引擎独立进程（`apps/pixel`，Fastify，仅 localhost）：画布算子 + pngjs 出 PNG + JSON 存档，不碰共享 SQLite；agent 经 `HttpPixelClient` 驱动，拆进程让 agent 重启不丢画布（#365）                                                                                                                                                                                                                                                 |
| `@sparkle/gba-service`       | GBA 模拟器独立进程（`apps/gba`，Fastify，仅 localhost）：内嵌 mGBA libretro WASM 核心（retroemu + romdev-platform-gba，npm pin 零运行时下载），前台=真机速率实时帧循环 / 后台=冻结，press 逐帧按键计划 + PNG 截图回传；电池存档 + ROM 元数据落自持 `data/gba/gba.db`（Prisma/SQLite），ROM 字节存 OSS；GL 原生依赖以 vendor/stubs 剥除（#541）                                                                                            |
...
| `@sparkle/image`             | 图片归一化叶子包（仅依赖 sharp，#556）：`normalizeImageForLlm`（进 LLM 上下文 / vision 前的直通 / 缩放 1568 / 极端长图切片 ≤6 片）+ `clampToApiLimit`（7900px + 40MP wire 保险丝）；agent（read_resource）/ napcat（vision 咽喉点）/ llm-client（claude provider）消费                                                                                                                                                                    |
...
| `@sparkle/spire-api`         | sparkle-spire 进程契约包：run/start、run/action、run/state、reference 四条逐字段 schema；服务端 state-view 与 agent 门面类型同源派生（#279 PR2）                                                                                                                                                                                                                                                                                          |
...
Sparkle 不是「QQ 群聊机器人」，而是一个有自己生活的 Agent。各类输入（QQ 消息、RSS、定时任务）在架构上平级，统一进入「手机 OS」模型：
...
- `apps/napcat` 自带独立的 `data/napcat/napcat.db`（自有 Prisma schema `apps/napcat/prisma/`，#539）：napcat_event / napcat_qq_message / napcat_event_outbox / image_asset 四表随进程独占，console 查询走 `@sparkle/napcat-api` 契约路由。
...
- PM2（`ecosystem.config.cjs`）托管以下进程：`sparkle-agent`（Fastify，Agent 运行时 + 活内存接口，默认 20003）、`sparkle-console`（管理台后端，前端只读查询聚合，#539 起零 DB 依赖，默认 20006）、`sparkle-gateway`（`apps/gateway`，纯反代：按前缀把 `/api/*` 分流到 console/agent、`/auth/*` 到 llm、`/metric/query` 到 metric，其余转 `sparkle-web`，默认 20004）、`sparkle-web`（`apps/web`，管理台前端独立进程，自持静态托管，默认 20016，仅 localhost；#578 起从 gateway 拆出，前端产物不再于构建期装配进网关，两者生命周期独立——`app:deploy web` 不动网关、`app:deploy gateway` 不动前端）、`sparkle-oss`（对象存储，默认 20005，仅 localhost）、`sparkle-browser`（`apps/browser`，持有 CloakBrowser，默认 20007，仅 localhost；`cwd` 固定仓库根，agent 重启不杀浏览器，`app:deploy agent` 不触及它，见 #173；#539 删 `browser_credential` 废表后零持久化、不开任何 SQLite）、`sparkle-llm`（`apps/llm`，LLM + OAuth 凭据网关，默认 20009，仅 localhost；持有 provider + callback server + 刷新 timer + 经 `SchedulerClient` 跑 Claude Files 缓存每日 GC（`claude_file_cache` 按 `last_used_at` idle 回收远端文件 + 本地行，#433），`app:deploy agent` 不触及它；#539 起独占 `data/llm/llm.db`（自带 retention 清理），不碰主库；llm 库迁移由 deploy.sh 单停 sparkle-llm 执行）、`sparkle-metric`（`apps/metric`，metric 摄取 + metric 图表查询，默认 20010，仅 localhost；agent fire-and-forget HTTP 上报；#475 P1 起独占 `data/metric/metric.duckdb`，不开共享 SQLite，主库迁移不需停它）、`sparkle-spire`（`apps/spire`，杀塔式卡牌游戏引擎，默认 20011，仅 localhost；`cwd` 固定仓库根让存档 `data/spire/` 落仓库根，`app:deploy agent` 不触及它，agent 重启不打断对局，见 #234）、`sparkle-pixel`（`apps/pixel`，像素画渲染引擎，默认 20012，仅 localhost；`cwd` 固定仓库根让存档 `data/pixel/` 落仓库根，`app:deploy agent` 不触及它，agent 重启不丢画布，见 #365）、`sparkle-napcat`（`apps/napcat`，NapCat 接入独立进程，默认 20013，仅 localhost；持有到 NapCat 的 WS 长连接 + 出站 RPC + 入站 SSE + vision/OSS 存档 + napcat 表落库，`cwd` 固定仓库根，`app:deploy agent` 不触及它，agent 重启不打断 QQ 连接，见 #347；#539 起独占 `data/napcat/napcat.db`（napcat_event / napcat_qq_message / outbox / image_asset + 自带 retention 清理），不碰主库；napcat 库迁移由 deploy.sh 单停 sparkle-napcat 执行）、`sparkle-scheduler`（`apps/scheduler`，通用定时调度薄时钟，默认 20014，仅 localhost；调度状态纯内存派生态、执行历史独占 `data/scheduler/scheduler.db`（#493），agent 与 sparkle-llm 经 `SchedulerClient` 注册 + SSE 收 tick，`app:deploy agent` 不触及它，agent 重启不打断计时节奏，见 #428）、`sparkle-gba`（`apps/gba`，GBA 模拟器独立进程，默认 20015，仅 localhost；内嵌 mGBA WASM 核心，前台实时帧循环 / 后台冻结，`cwd` 固定仓库根让 `data/gba/`（ROM 元数据 + 电池存档 sqlite）落仓库根，ROM 字节存 OSS，`app:deploy agent` 不触及它，agent 重启不丢模拟器热状态与存档，见 #541；优雅关停落 savestate 快照（`resume_state` 单行，消费即删），`app:deploy gba` 重启后 unserialize 接续停机现场（含前后台）实现无感重启，agent 侧 gba-client 另有连接被拒短重试吃掉重启窗口；crash 无快照，落回「断电 + 电池存档」语义）。agent 库 `data/agent/agent.db`（原主库 kagami.db）自 #539 起由 **sparkle-agent 独占**（console 零 DB、经各服务查询路由聚合；`apps/spire` / `apps/pixel` / `apps/scheduler` / `apps/metric` / `apps/browser` / `apps/gba` / `apps/napcat` / `apps/llm` 不入共享库，spire/pixel 存档走 JSON、scheduler 独占 `data/scheduler/scheduler.db`、metric 独占 DuckDB、browser 零持久化 #539、gba 自持 `data/gba/gba.db`、napcat 独占 napcat.db #539、llm 独占 llm.db #539）。
- 卫星进程（console / oss / browser / llm / metric / spire / pixel / gba / napcat / scheduler）统一经 `@sparkle/kernel` 的 `runService` 启动（issue #274）：全局 `uncaughtException` / `unhandledRejection` 兜底（记日志后 exit(1) 交 PM2 重启）、信号驱动优雅关停 + 10s 强退兜底、绑定地址一律 `127.0.0.1`（绑定是代码级安全决策；config 的 `services.*.host` 语义是 reachable host）。gateway 是唯一绑 `0.0.0.0` 的前门（裸 node:http，自带同款兜底）。所有进程的 `GET /health` 统一为 shared 的 `{ status: "ok", timestamp }` 形状。
