...
# ── 单服务模式：pnpm app:stop <agent|console|gateway|web|oss|browser|llm|metric|feishu|scheduler> ──
...
case "$SERVICE" in
  ...
  feishu) PM2_NAME="sparkle-feishu" ;;
  scheduler) PM2_NAME="sparkle-scheduler" ;;
  *)
    echo "用法: pnpm app:stop [<agent|console|gateway|web|oss|browser|llm|metric|feishu|scheduler>]" >&2
    echo "  无参：停掉所有进程。" >&2
    echo "  带服务名：只停该服务。" >&2
    exit 1
    ;;
