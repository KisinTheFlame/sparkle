  ...
  it("should render the root context summary reminder", () => {
    expect(createRootContextSummaryReminderMessage()).toEqual({
      ...
      content: [
        ...
        "跨轮仍成立的背景，当前仍在延续的线索，关键对象，Sparkle 自己的判断与倾向，已经做过的关键动作及结果，以及后续还可以继续展开的点。",
        "在原始上下文确有依据时，尽量保留具体对象与关系、决定及理由、因果链、推进到的位置、成功与失败的结果、承诺与约束、尚未确认的不确定性、能帮助续接的极短原话或线索，以及 Sparkle 实际形成的判断与倾向。",
        "摘要长度应随有效信息量自适应。信息充足时通常写到约 4000 到 8000 个中文字符；复杂上下文可以更长，不要因为达到这个范围就截断重要信息。信息较少时可以自然缩短，不要为了凑字数重复或填充内容。",
        "摘要使用 Markdown 二级标题，按固定顺序组织为：`## 持续背景`、`## 仍在延续的线索`、`## 关键对象`、`## Sparkle 的判断与倾向`、`## 已做动作与结果`、`## 还可以继续展开的点`。",
        "每节可以使用多个原子化条目；一个条目只承载一组紧密相关的信息，并写清对象、变化、原因或当前落点，便于以后准确合并和更新。",
        "`## 持续背景` 保留跨轮仍重要的事实、关系、承诺、约束、长期判断。",
        "`## 仍在延续的线索` 保留当前还没完的事情，可以是进行中的任务、聊天话题、阅读线索、判断链或其他活动；写清它最近推进到了哪。",
        "`## 关键对象` 按“为什么现在仍重要”来写，可以包括人、群、文章、帖子、事件、问题、目标或别的关键对象。",
        "`## Sparkle 的判断与倾向` 写 Sparkle 打算优先推进什么、对哪些方向有保留或倾向搁置、哪些判断已经形成、哪些还悬而未决。",
