import { isRecord, toJsonRecord } from "@sparkle/shared/utils";
import type {
  AppLogItem,
  InsertAppLogItem,
  LogDao,
...
export class PrismaLogDao implements LogDao {
  ...
  public async countByQuery(input: QueryAppLogListFilterInput): Promise<number> {
    ...
    const rows = await this.database.$queryRaw<Array<{ total: bigint | number | string }>>(
      Prisma.sql`
  ...
  public async listByQueryPage(input: QueryAppLogListPageInput): Promise<AppLogItem[]> {
    ...
    return rows.map(row => ({
      ...
      message: row.message,
      metadata: toJsonRecordValue(row.metadata),
...
function toJsonRecordValue(value: unknown): Record<string, unknown> {
  // SQLite 经原生 SQL 返回的 JSON 列可能是字符串，先尝试解析再归一成 record。
  if (typeof value === "string") {
    try {
      return toJsonRecord(JSON.parse(value));
    } catch {
      return { value };
    }
  }
...
/**
 * SQLite 不支持 Prisma 的 JSON 过滤与大小写不敏感匹配，因此 app_log 的列表/计数走原生 SQL：
 * `source` 用 SQLite 的 `metadata ->> 'source'` 提取，模糊匹配用 `LIKE`。
 */
